...
class MRRecommendation(MRJob):
    ...
    def reducer_similarity(self, item_pair, rating_pairs):
        ...
        for rating1, rating2 in rating_pairs:
            ...
            i = i + 1
        jaccard_similarity = i * dot_product / (((norm_item1)) + ((norm_item2)) - dot_product) if norm_item1 and norm_item2 else 0
        yield item_pair, jaccard_similarity


class MRRecommendation2(MRJob):
    def steps(self):
        return [
            MRStep(mapper=self.mapper_preprocess, reducer=self.reducer_preprocess)
        ]
